...
# Assign taxonomy with the R package[Taxonomzir](https://cran.r-project.org/web/packages/taxonomizr/index.html). This requires some previous setup and long downloads.
```library(taxonomizr)```
...
A few id's are missing. NCBI's [batch entrez](https://www.ncbi.nlm.nih.gov/sites/batchentrez) is helpful for checking those manually. Remove any results deemed contamination by the analysis.

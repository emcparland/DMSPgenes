...
#############################################################################
CIRCLE_SIZE_PREFIX_IN_DATA_FILE="Class_"
CIRCLE_DYNAMIC_COLOR_PREFIX_IN_DATA_FILE="Color_"
MAP_TITLE<-ASSIGN_MAP_TITLE
...
FONT_SIZE <- LABELS # set it to 0 to see no labels
FONT_COLOR="black"
...
CIRCLE_COLOR_LOW="blue"
CIRCLE_COLOR_HIGH="red"
...
# scaling points
MIN_POINT_SIZE <- 0.1 #0.2
MAX_POINT_SIZE <- 10 #15
MARGIN_MIN_LAT <- -5
MARGIN_MAX_LAT <- 5
MARGIN_MIN_LON <- -5
MARGIN_MAX_LON <- 5
...
gen_blank_world_map_simple <- function(df) {
  ...
  max_lon <- max(df$Lon) + MARGIN_MAX_LON
    ...
    min_lat <- -60 + MARGIN_MIN_LAT
    max_lat <- 60 + MARGIN_MAX_LAT
    min_lon <- -150 + MARGIN_MIN_LON
    max_lon <- 150 + MARGIN_MAX_LON
  }
...
add_mag_abundances <- function(plot_object, df, mag, mag_color=NULL, color_low=NULL, color_high=NULL, alpha=0.2, labels = TRUE){
  if (!is.null(mag_color)) {
    plot_object <- plot_object + geom_point(data = df,
                                            ...
                                            stroke = 0,
                                            alpha=ALPHA)
    #plot_object <- plot_object + scale_colour_gradientn(colors=rev(rainbow(5)),limits=c(0,1))
    plot_object <- plot_object + scale_colour_gradientn(colors=rev(c('#d7191c','#fdae61','#ffffbf','#abd9e9','#2c7bb6')),limits=c(0,1))
  } else {
    plot_object <- plot_object + geom_point(data = df,
                                            aes_string(x="Lon", y="Lat",
                                                       group=mag,
  ...
  plot_object <- plot_object + scale_size(range=c(MIN_POINT_SIZE,
                                                  MAX_POINT_SIZE),limits = c(CIRCLE_LOWER_LIM,CIRCLE_UPPER_LIM))
  plot_object<- plot_object + ggtitle(MAP_TITLE)
  return(plot_object)
}
...
for (MAG in MAGs){
  ...
  ggsave(paste(MAG, '.eps',sep=''),width=PDF_WIDTH,height=PDF_HEIGHT)
  dev.off()
  cat(sprintf(" ... OK\n"))
}
